...
if($_SESSION['action']==0){
	...
	if($step==0.5){
		...
		header('location: viewpriceitems.php?id='.$params['mid'].'&from='.$_SESSION['from'].'&to_page='.$_SESSION['to_page']);
		die();
...
if($_SESSION['action']==1){
	...
	if($step==0.5){
		...
		$_SESSION['articul']=SecStr($_POST['articul']);
		...
		$_SESSION['firmid']=abs((int)$_POST['firmid']);
	...
	if($step==2.5){
		...
		foreach($photos as $n=>$item){
			...
			$params['articul']=$_SESSION['articul'];
		...
		header('Location: viewpriceitems.php?id='.$_SESSION['mid'].'&from='.$_SESSION['from'].'&to_page='.$_SESSION['to_page']);
		die();
	}
}
...
?>
	...
	<?if($_SESSION['action']==0){?>
	<h1>Добавление указанного количества товаров</h1>
	<?}
	if($_SESSION['action']==1){?>
	<h1>Добавление товаров по фото из папки</h1>
	<?}?>
	
	
	<form action="add_goods.php" method="post" id="inpp" name="inpp">
	...
	<strong>Количество добавляемых товаров:</strong><br>
	...
	Артикул товара:<br />
		<input name="articul" type="text" size="40" maxlength="255" value="" /><p />			
	...
	<strong>Фирма-производитель:</strong>&nbsp;
	<select name="firmid" id="firmid">
		<option value="0" <?if(!isset($_SESSION['firmid'])) echo 'SELECTED'; else if($_SESSION['firmid']==0) echo 'SELECTED';?>>-не определена-</option>
		<?
		$fg=new FirmsGroup();
		if(isset($_SESSION['firmid'])) echo $fg->GetItemsOptByLang_id($_SESSION['firmid'],'name',LANG_CODE);
		else echo $fg->GetItemsOptByLang_id(0,'name',LANG_CODE);
		
		?>
	</select>
	
	<a href="viewfirms.php" target="_blank">Все фирмы...</a>
	<p>
		

			Местонахождение товара:<br>
...
Артикул товара:<br />
		<input name="articul" type="text" size="40" maxlength="255" value="<?if(isset($_SESSION['articul'])) echo htmlspecialchars(stripslashes($_SESSION['articul']));?>" /><p />	
	...
	<p>
		...
		<?if($step==1){
			...
			<tr>
				...
				<strong>Название товара: </strong><?=stripslashes($_SESSION['name'])?><p>
				<strong>Артикул товара: </strong><?=stripslashes($_SESSION['articul'])?><p>
				
				<strong>Фирма-производитель: </strong><?
				$fi=new FirmItem();
				$firm=$fi->GetItemById($_SESSION['firmid'],LANG_CODE);
				if($firm!=false) echo stripslashes($firm['name']);
				else echo 'не определена';
				?><p>
				
				<strong>Малый текст к товару: </strong><?=stripslashes($_SESSION['small_txt'])?><p>
				<strong>Текст к товару: </strong><?=stripslashes($_SESSION['big_txt'])?><p>
				<strong>Порядок показа товара: </strong><?=stripslashes($_SESSION['ord'])?><p>
	...
	<script  language="JavaScript">  
	 ...
	 <?
	if($_SESSION['action']==0){?>
	  frmvalidator.addValidation("cou","req","Заполните поле Количество добавляемых товаров!"); 
	  frmvalidator.addValidation("cou","num","В поле Количество добавляемых товаров допустимы только цифры!"); 
	  ...
	  frmvalidator.addValidation("name","req","Заполните поле Название товара!"); 
	 <?}?>	  
	 <?
