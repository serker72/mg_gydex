...
require_once('../classes/claimgroup.php');
...
if($rights_man->CheckAccess($global_profile['login'], $global_profile['passw'], 'r', 11)) {
  ...
  if(isset($_POST['Update'])||isset($_POST['Update1'])){
	  ...
	  header('Location: viewclaims.php?from='.$from.'&to_page='.$to_page);
	  die();
  }
}else{
	header('Location: no_rights.php');
...
$smarty->assign("SITETITLE",'«а€вки с сайта - '.SITETITLE);
...
?>
	...
	echo $og->GetItems(0, 'claim/items.html', $from, $to_page);
