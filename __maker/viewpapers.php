...
if(isset($_POST['Update'])||isset($_POST['Update1'])){
	...
	header('Location: viewpapers.php?id='.$id.'&from='.$from.'&to_page='.$to_page);
	die();
...
$_context->AddContext(new ContextItem( 20, 'a', "", "Создать статью", "ed_paper.php?action=0&mid=".$id, false , $global_profile  ));
$_context->AddContext(new ContextItem( 20, 'r', "", "Все статьи", "viewpapers.php?sortmode=0&id=".$id, false , $global_profile  ));
...
$_context->AddContext(new ContextItem( "", '',  "", "Справка", "viewpapers.html", true , $global_profile  ));
...
$_bc->AddContext(new BcItem('Список статей', basename(__FILE__)));
...
?>
	...
	$pl=new PapersGroup();
	$pl->SetTemplates('papers/items.html','tpl/papers/itemsrow.html','','tpl/to_page.html');
