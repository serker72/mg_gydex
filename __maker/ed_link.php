...
$smarty->assign("SITETITLE",'Редактирование ссылки - '.SITETITLE.'');
...
?>
	...
	<?
	//навигация
	echo $razd->DrawNavig($mid, LANG_CODE,0,'правка ссылки');
	?>
	...
	?>
			...
			<strong>Местонахождение ссылки:</strong><br>
	...
	<tr>
		...
		<input class="unbord" type="radio" name="use_simple_code" value="0" <?if($action==0) echo 'checked'; else if($action==1){  if($photo['use_simple_code']==0) echo 'checked';}?>><strong>Использовать данные:</strong><p>
    ...
    <div id="full_upload">
		...
		<a href="javascript: //" class="" onclick="window.open('photofolder.php?mode=2','photo_manager', 'width=1100, height=700,scrollbars=yes,resizable=yes'); document.forms.inpp.photo_small.value=''; document.forms.inpp.photo_small.focus();">выбрать фото</a><br />
	...
	CKEDITOR.replace( 'small_txt',
                ...
                <label for="vnedr">Внедрение:</label><br />
		...
		<input class="unbord" type="radio" name="use_simple_code" value="1" <?if(($action==1)&&($photo['use_simple_code']==1)) echo 'checked';?>><strong>Использовать код:</strong><p>
