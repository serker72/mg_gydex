...
if(isset($_POST['Update'])||isset($_POST['Update1'])){
	...
	header('Location: viewlinks.php?id='.$id.'&from='.$from.'&to_page='.$to_page);
	die();
...
$smarty->assign("SITETITLE",'Список ссылок - '.SITETITLE.'');
...
$_context->AddContext(new ContextItem( 19, 'a', "", "Создать ссылку", "ed_link.php?action=0&mid=".$id, false , $global_profile  ));
$_context->AddContext(new ContextItem( 19, 'r', "", "Все ссылки", "viewlinks.php?sortmode=0&id=".$id, false , $global_profile  ));
...
$_context->AddContext(new ContextItem( "", '',  "", "Справка", "viewlinks.html", true , $global_profile  ));
...
$_bc->AddContext(new BcItem('Список ссылок', basename(__FILE__)));
...
?>
	...
	$pl=new LinksGroup();
	$pl->SetTemplates('links/items.html','tpl/links/itemsrow.html','tpl/links/itemsrow_code.html','tpl/to_page.html');

	echo $pl->GetItemsById($id, 0,$from,$to_page);
