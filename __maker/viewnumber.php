...
if($rights_man->CheckAccess($global_profile['login'], $global_profile['passw'], 'r', 11)) {
  ...
  if(isset($_POST['Update'])||isset($_POST['Update1'])){
	  ...
	  header('Location: viewnumber.php?id='.$id.'&from='.$from.'&to_page='.$to_page);
	  die();
  }
}else{
	header('Location: no_rights.php');
...
$smarty->assign("SITETITLE",'Gydex в числах. '.SITETITLE);
...
?>
	...
	echo $og->GetItems(0,  'num/items.html', $from, $to_page);
