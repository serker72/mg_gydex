...
require_once('distr_usersgroup.php');
...
//матрица доступа по пользователям
class DistrMatrUs{
	protected $_groups;
	protected $_group;
	protected $_rights;
	...
	function __construct(){
		...
		$this->_objects->SetPageName('discr_matrix_user.php');
		$this->_users= new DistrUsersGroup;
		$this->_users->SetPageName('discr_matrix_user.php');
		$this->_user= new DistrUserItem;
	}
	...
	public function Draw($template,$is_ajax=false, $gfrom=0, $ofrom=0, $per_page=ITEMS_PER_PAGE){
		...
		if(isset($global_profile['login'])&&isset($global_profile['passw'])&&($this->_rights_manager->CheckAccess($global_profile['login'], $global_profile['passw'], 'r', 5))) {
		
		$groups_arr= $this->_users->GetItemsArr($gfrom, $per_page, $gnavi, Array('ofrom'=>$ofrom));}else $groups_arr=Array();
		...
		if(isset($global_profile['login'])&&isset($global_profile['passw'])&&($this->_rights_manager->CheckAccess($global_profile['login'], $global_profile['passw'], 'r', 1))) {
			$objects_arr= $this->_objects->GetItemsArr($ofrom, $per_page, $onavi, Array('gfrom'=>$gfrom));} else $objects_arr=Array();
		foreach($groups_arr as $k=>$v){
				$new_objects=Array();
				foreach($objects_arr as $kk=>$vv){
					//права
					$vv['active_rights']=Array(); 
					if(isset($global_profile['login'])&&isset($global_profile['passw'])&&($this->_rights_manager->CheckAccess($global_profile['login'], $global_profile['passw'], 'r', 4))) $vv['active_rights']=$this->_rights_manager->GetActiveRightsUser($v['id'], $vv['id']);
					$vv['active_rights_len']=count($vv['active_rights']);
					
					$vv['inactive_rights']=Array(); 
					if(isset($global_profile['login'])&&isset($global_profile['passw'])&&($this->_rights_manager->CheckAccess($global_profile['login'], $global_profile['passw'], 'r', 4))) $vv['inactive_rights']=$this->_rights_manager->GetInactiveRightsUser($v['id'], $vv['id']);
