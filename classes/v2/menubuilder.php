...
class MenuBuilder{
	...
	function __construct($place_kind=2, $res=NULL){
		...
		for($i=0; $i<$rc; $i++){
			...
			if($f['object_id']==0){
				$item=new MenuAllItem($f['id'], $f['parent_id'], $f['object_id'], $f['name'], $f['description'], $f['url'], $f['ord'],  $f['is_pic'], $f['pic'], $f['module_constant']);  
				$item->SetAuthResult($res);
			
			}else{
				
				$item=new MenuSecItem($f['id'], $f['parent_id'], $f['object_id'], $f['name'], $f['description'], $f['url'], $f['ord'],  $f['is_pic'], $f['pic'], $f['module_constant']);  
				$item->SetAuthResult($res);
			}
