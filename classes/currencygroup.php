...
require_once('curruse.php');

// группа валют
class CurrencyGroup  extends AbstractLangGroup{
	...
	public function GetItems($from=0,$to_page=ITEMS_PER_PAGE){
		...
		for($i=0;$i<$rc;$i++){
			...
			$all_[]=Array('itemno'=>$f['id'], 'is_base_shop'=>$f['is_base_shop'], 'is_base_rate'=>$f['is_base_rate'],'kurs'=>sprintf("%.6f",$f['rate']), 'usein'=>$usest, 'used'=>$used, 'nameitems'=>$names, 'valitems'=>$params);
		}
