	...
	public function GetFileList($from=0,$per_page=PHOTOS_PER_PAGE,$allowable='jpg|jpe|jpeg|gif|png|wbm',$params=Array()){
		...
		foreach($this->files as $k=>$v){
			if(($cter>=$from)&&($cter<$totalcount)) {
				...
				if(isset($_SESSION[$this->sessionname][$v['rel_path']])) {
				...
				$sz = @GetImageSize($this->base_path."/".$v['rel_path']);
				
				if($sz){
					$w=$sz[0];
					$h=$sz[1];	
					$kbsize=ceil(filesize($this->base_path."/".$v['rel_path'])/1024);
					...
					$durl = 'photodrawer.php?w1='.$w1.'&h1='.$h1.'&picname='.$v['rel_path'];
				}else{
					$durl='../img/01.gif';
					$w1=$this->preview_size[1];
					$h1=$this->preview_size[0];
					$w='?';
					$h='?';
				}
				
				$controls=$this->DrawFormControls($v);
				
				$cells[]=Array(
					'td_width'=>ceil(100/PHOTOS_PER_ROW),
					//'download_path'=> 'srv/get.php?name='.$v['rel_path'], 
					'download_path'=> $this->root_path.$v['rel_path'], 
					'pict'=>$durl,
					'img_w'=>$w1,
					'img_h'=>$h1,
					'real_w'=>$w,
		...
		$res=Array(
			...
			'scripts'=>$this->DrawFormControlScript()
		);
		return $res;
	}
